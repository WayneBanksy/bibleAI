...
async def test_escalate_path_no_llm():
    ...
    with _no_db_patches(), \
         patch("app.pipeline._llm_stub", side_effect=AssertionError("LLM must not be called on escalate")):
        await run_pipeline(
...
async def test_escalate_violence_path():
    ...
    with _no_db_patches(), \
         patch("app.pipeline._llm_stub", side_effect=AssertionError("LLM must not be called")):
        await run_pipeline(
            session_id=uuid.uuid4(),
            user_id=uuid.uuid4(),
            user_message_id=uuid.uuid4(),
